...
def retrieve_items(obj, routes):
    yield from (functools.reduce(operator.getitem, [obj, *route]) for route in routes)
...
class TestPick:
    ...
    def test_picked_objects_by_identity(self, data, pred, routes):
        ...
        for actual, expected in itertools.zip_longest(
            result, expected_items, fillvalue=sentinel
        ):
...
class TestCollectionHandling:
    ...
    @pytest.mark.parametrize(
        ...
        (
            pytest.param(
                LIST_5_LEVELS,
                [bytearray(b"2"), "4", 3.5, b"1", "0", "2", b"3", "1", 2],
                id="list 5 levels - no collections",
            ),
            pytest.param(
                DICT_5_LEVELS,
                ["0_value1", "2_value1", "4_value", "4_value2", "1_value2"],
                id="dict 5 levels - no collections",
            ),
        ),
    )
    def test_collections_excluded_optionally(self, data, expected):
        assert list(pick(data, lambda x: True, collections=False)) == expected
    ...
    def test_collections_in_dict_keys_excluded_optionally(self):
        data = {"1": (), (0, 1): 2}
        result = pick(data, lambda x: True, collections=False, dict_keys=True)
...
class TestFunctionsAndPredicates:
    @pytest.mark.parametrize(
        "data, pred, expected",
        (
            pytest.param(FLAT, bool, [62, True, "food", "foo"], id="flat - bool"),
            pytest.param(
                NESTED_LIST,
                bool,
                [
            ...
            pytest.param(FLAT, is_str, ["food", "", "foo"], id="flat - is str"),
            pytest.param(
                LIST_TUPLE,
                is_tuple,
            ...
            pytest.param(
                NESTED_LIST, is_int, [1, 2, 3, 4, 5], id="nested list - is int"
            ),
            pytest.param(
                FLAT,
                lambda obj: hasattr(obj, "count"),
                ["food", "", [], "foo"],
                id="flat - has .count",
            ),
            pytest.param(
                NESTED_DICT,
                lambda obj: hasattr(obj, "append"),
                [[], [1, [2, [3, {}]]], [2, [3, {}]], [3, {}], [], []],
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            pytest.param(FLAT, lambda obj: len(obj) == 0, TypeError, id="flat - len 0"),
            pytest.param(
                LIST_TUPLE,
            ...
            pytest.param(
                FLAT, lambda obj: "foo" in obj, TypeError, id='flat - contains "foo"'
            ),
            pytest.param(
                LIST_TUPLE,
                lambda obj: 3 in obj,
            ...
            pytest.param(
                FLAT, lambda obj: obj % 2 == 0, TypeError, id="flat - even number"
            ),
        ),
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            pytest.param(
                FLAT, Predicate(lambda obj: obj[2]), ["food", "foo"], id="flat - item 2"
            ),
            pytest.param(
                NESTED_LIST,
                Predicate(lambda obj: obj[1]),
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            pytest.param(
                ...
                ["foot", "foobar", {"foo": "bar", "bar": "fool"}, "fool", "good food"],
                id='strings - contains "foo"',
            ),
            pytest.param(
                DICT_LIST, Predicate(lambda n: 2 <= n <= 4), [2], id="dict list - 2-4"
            ),
    ...
    @pytest.mark.parametrize(
        ...
        (
            pytest.param(
                STRINGS,
                is_str,
                ["foot", "", "foobar", "foo", "bar", "bar", "fool", "good food"],
                id="strings - is str",
            ),
            pytest.param(
                STRINGS,
...
class TestStringsAndBytesLike:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            pytest.param((0, [[bytearray([1, 2])], b"12"], 1), [0, 1], id="nested"),
            pytest.param({bytes([1, 2]): bytearray(b"\003\004")}, [], id="dict"),
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            pytest.param(
                (0, [[bytearray([1, 2])], b"12"], 1), [0, 1, 2, 49, 50, 1], id="nested"
            ),
            pytest.param({bytes([1, 2]): bytearray(b"\003\004")}, [3, 4], id="dict"),
            pytest.param(b"", [], id="top-level bytes empty"),
...
class TestFlattening:
    @pytest.mark.parametrize(
        "data, expected",
        (
            pytest.param(FLAT, [62, 0.0, True, "food", "", None, "foo"], id="flat"),
            pytest.param(NESTED_LIST, [1, 2, 100.0, 3, "Py", 4, 5], id="nested list"),
            pytest.param(DICT_LIST, [2, "3", 5], id="dict list"),
            pytest.param(
                LIST_TUPLE, [None, 1, 2, 3, 3, 0, "foo", "bar"], id="list tuple"
            ),
            pytest.param(NESTED_DICT, [1, 2, 3], id="nested dict"),
            pytest.param(
                LIST_5_LEVELS,
                [bytearray(b"2"), "4", 3.5, b"1", "0", "2", b"3", "1", 2],
                id="list 5 levels",
