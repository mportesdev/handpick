...
class TestFromFunctionFactoryMethod:
    ...
    @pytest.mark.parametrize(
        "function_or_predicate", (lambda x: x > 0, Predicate(lambda x: x > 0))
    )
...
class TestOverloadedOperators:
    ...
    def test_predicate_and_predicate(self):
        ...
        data = [("1", [0, ["py"]]), {"f": {(2,), "2"}}, {b"A"}, "foo", {3: "foo"}]
    ...
    def test_predicate_and_function(self):
        ...
        data = [("2", [2]), {"long": {(2,), "2"}}, range(2), "long", {2: "long"}]
    ...
    def test_predicate_or_predicate(self):
        ...
        data = [("1", [None, 9.51]), {"": {2.0, "2"}}, range(5, 7), "2", {3: True}]
        result = list(pick(data, can_be_int))
        assert result == [9.51, 2.0, 5, 6, True]
...
class TestPredicateFactories:
    @pytest.mark.parametrize(
        ...
        (
            ...
            pytest.param(
                NESTED_DICT, tuple, [(), (), ({}, [], ()), ()], id="nested dict - tuple"
            ),
            pytest.param(
                NESTED_DICT,
                (list, int),
    ...
    @pytest.mark.parametrize(
        "data, type_or_types, expected",
        (
            pytest.param(FLAT, str, [62, 0.0, True, None, [], {}], id="flat - str"),
            pytest.param(
                FLAT,
                (list, dict),
...
class TestBuiltinPredicates:
    ...
    @pytest.mark.parametrize("json_file", ("list_of_int.json", "dict_of_int.json"))
